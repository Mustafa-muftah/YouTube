...
export const SearchPage: React.FC = () => {
	...
	const { searchList, searchInputValue, NextpageToken } = useTypeSelector(
		(state) => state.search
	);
	...
	const renderChannelOrVideo = (list: SearchListType[]): ReactNode => {
		return list.map((item, index: number) => {
			if (item.id.videoId) {
				return <Video key={`${item.id.videoId}${index}`} videoInfo={item} />;
			} else {
				return (
					<Channel key={`${item.id.channelId}${index}`} channelInfo={item} />
				);
			}
		});
	};
